...
class Predictit(commands.Cog):
    """Gets information about and searches for predictit markets."""

    @commands.command()
    async def predictit(self, ctx):
        """This does stuff!"""
        # Your code will go here
        await ctx.message.add_reaction("🐻")
        await ctx.trigger_typing()
        predictit_string = ctx.message.content[11:]
        try:
            try:
                int(predictit_string)
                req = requests.get(
                    f'https://www.predictit.org/api/marketdata/markets/{ctx.message.content[11:]}').json()
                if req is None:
                    await ctx.channel.send(
                        f"Predictit returned no data for that contract id. You can get a contract's id from the contract's URL: <https://www.predictit.org/markets/detail/**4319**/Will-Donald-Trump-be-impeached-by-year-end-2019>")
                    return
                message_list = [f"{ctx.author.mention}:", f"{req['name']}",
                ...
                if message_list != "```":
                    message_list.append("```")
                graph = get_predictit_graph(ctx.message.content[11:], req['name'])
                temp_file = dFile(fp=graph, filename=f"{predictit_string}.png", spoiler=False)
                await ctx.send(content="\n".join(message_list), file=temp_file)
                graph.close()
            except ValueError:
                req = requests.get(f'https://www.predictit.org/api/marketdata/all/').json()
                if req is None:
                    await ctx.send("Predictit returned no data to search.")
                    return
                search_term = predictit_string.lower()
                message_list = [f"{ctx.author.mention}:", f"Search results for `{search_term}`:", "```"]
                header = f"CONTRACT NAME (ID){' ' * 48}(ID)    LYP (CHANGE) BYP BNP"
                message_list.append(header)
                for market in req['markets']:
                    for contract in market['contracts']:
                        if search_term in contract['longName'].lower() or search_term in market['name'].lower():
                            if len(contract['longName']) > 65:
                                contract['longName'] = f"{contract['longName'][:62]}..."
                            contract_string = f"{contract['longName']} ({market['id']})"
                            if contract['lastTradePrice'] is None:
                                contract['lastTradePrice'] = 0
